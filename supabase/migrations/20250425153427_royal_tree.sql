/*
  ...
  1. Changes
    - Remove title_en field since it's redundant with title
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Allow authenticated users to manage chapters" ON chapters;
  DROP POLICY IF EXISTS "Allow public read access to chapter content" ON chapter_content;
  DROP POLICY IF EXISTS "Allow authenticated users to manage chapter content" ON chapter_content;
  DROP POLICY IF EXISTS "Allow public read access to chapter questions" ON chapter_questions;
  DROP POLICY IF EXISTS "Allow authenticated users to manage chapter questions" ON chapter_questions;
